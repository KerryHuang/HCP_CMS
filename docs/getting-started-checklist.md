...
### A2. 瀏覽各頁面
依序點擊左側導覽列每一項：
- [ ] 📊 儀表板 — 看到 4 個 KPI 卡片（全部顯示 0）
- [ ] 📋 案件管理 — 看到空的案件列表 + 篩選器 + 按鈕
- [ ] 📚 KMS 知識庫 — 看到搜尋框 + 空的結果表
- [ ] 📧 信件處理 — 看到來源選擇、日期篩選、進度條
- [ ] 🔧 Mantis 同步 — 看到 API 選擇、URL 輸入框
- [ ] 📊 報表中心 — 看到報表類型選擇、年月設定
- [ ] 📏 規則設定 — 看到規則類型下拉選單
- [ ] ⚙️ 系統設定 — 看到使用者、資料庫、備份設定區域

### A3. 測試快捷鍵
- [ ] 按 `Ctrl+K` — 應自動跳到 KMS 知識庫頁面
...
5. 再新增一條：
   ...
   - Value: `ERP`
   - Priority: `2`
- [ ] 表格顯示 2 條規則，WebLogic 排在前面（priority 小的優先）
